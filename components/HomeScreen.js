    ...
    return (
      ...
      >
        <View style={styles.col2}>
          ...
          {/* task info */}
          <View>
            <Text style={[styles.itemTitle, item.completed
            ? styles.crossText : undefined]}>{item.title}</Text>
            {
              // render description only if item has it
              item.description.length > 0 ? 
              <Text style={item.completed
            ? styles.crossText : undefined}>{item.description}</Text> : undefined
            }
          </View>
        </View>
        {/* task options */}
  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.mainArea}>        
        ...
        <Text style={[styles.sectionTitle, {textAlign:"center", color: "white", marginBottom: 5}]}>Create a New Task</Text>

        <Input placeholder="Task Name" style={styles.whiteText} onChangeText={(value) => setTaskName(value)}></Input>
        <Input placeholder="Task Description... (optional)" style={styles.whiteText} onChangeText={(value) => setTaskDesc(value)}></Input>
          <TouchableOpacity
          style={[
            styles.newTaskBtn,
            taskName.length == 0 ? { opacity: .2 } : { opacity: 1}, 
          ]}
          onPress={addNewTask}
